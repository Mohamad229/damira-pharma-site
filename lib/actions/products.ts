...
const CreateProductSchema = z.object({
  ...
  description: z.string().min(1, 'Product description is required').max(2000, 'Description is too long'),
...
const UpdateProductSchema = z.object({
  ...
  description: z.string().min(1, 'Product description is required').max(2000, 'Description is too long').optional(),
  type: z.enum(['SIMPLE', 'ADVANCED']).optional(),
  status: z.enum(['AVAILABLE', 'PIPELINE']).optional(),
  categoryId: z.string().optional().nullable(),
  therapeuticAreaId: z.string().optional().nullable(),
 ...
 */
async function ensureUniqueSlug(baseSlug: string, productId?: string): Promise<string> {
  let slug = baseSlug;
  let counter = 1;
...
export async function getProductById(
  ...
  try {
    ...
    const detail: ProductDetail = {
      id: product.id,
      name: product.translations.find((t) => t.locale === 'en')?.name || 'Untitled',
      type: product.type,
      status: product.status,
      description: product.translations.find((t) => t.locale === 'en')?.shortDescription || null,
      coverImage: product.coverImage,
...
export async function createProduct(
  ...
  try {
    ...
    if (!validatedInput.success) {
      ...
      const errorMessage =
        Object.values(errors).flatMap((e) => e || []).join(', ') ||
...
export async function updateProduct(
  ...
  try {
    ...
    if (therapeuticAreaId !== undefined) {
      updateData.therapeuticArea = therapeuticAreaId ? { connect: { id: therapeuticAreaId } } : { disconnect: true };
    }

    if (manufacturerId !== undefined) {
      updateData.manufacturer = manufacturerId ? { connect: { id: manufacturerId } } : { disconnect: true };
    }
 ...
 */
export async function getProductTypes(): Promise<ActionState & { data?: string[] }> {
  try {
    await requireAuth();
 ...
 */
export async function getProductStatuses(): Promise<ActionState & { data?: string[] }> {
  try {
    await requireAuth();
