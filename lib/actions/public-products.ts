...
export async function getPublicProducts(
  ...
  const pageSize = Math.max(1, Math.min(24, query.pageSize || DEFAULT_PAGE_SIZE));
  ...
  const coverImageUrls = await resolveCoverImageUrls(products.map((product) => product.coverImage));
    ...
    return {
      ...
      category: product.category
        ? {
            ...
            name: getLocalizedLabel(product.category.name, product.category.nameAr, locale),
          }
        : null,
      therapeuticArea: product.therapeuticArea
        ? {
            id: product.therapeuticArea.id,
            name: getLocalizedLabel(product.therapeuticArea.name, product.therapeuticArea.nameAr, locale),
          }
        : null,
...
export async function getPublicProductBySlug(
  ...
  const translation = getProductTranslation(product.translations, locale);
  const relatedProducts = await getRelatedPublicProducts(locale, product.id, product.categoryId, product.therapeuticAreaId);
...
export async function getPublishedProductSlugs(): Promise<PublishedProductSlug[]> {
...
async function getRelatedPublicProducts(
  ...
  const coverImageUrls = await resolveCoverImageUrls(related.map((item) => item.coverImage));
  ...
  return related.map((item) => {
    ...
    return {
      ...
      category: item.category
        ? {
            id: item.category.id,
            name: getLocalizedLabel(item.category.name, item.category.nameAr, locale),
          }
        : null,
      therapeuticArea: item.therapeuticArea
        ? {
            id: item.therapeuticArea.id,
            name: getLocalizedLabel(item.therapeuticArea.name, item.therapeuticArea.nameAr, locale),
          }
        : null,
...
function getLocalizedLabel(name: string, nameAr: string | null, locale: Locale) {
  if (locale === 'ar') {
    return nameAr || name;
  }
...
function resolveCoverImageValue(value: string | null, mediaMap: Map<string, string>) {
  if (!value) {
    return null;
  }
