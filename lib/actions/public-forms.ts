...
const PARTNERSHIP_SCHEMA = CONTACT_SCHEMA.extend({
  ...
  inquiryType: z.enum(['distribution', 'licensing', 'strategic', 'investment', 'other'], {
...
const PRODUCT_INQUIRY_SCHEMA = CONTACT_SCHEMA.extend({
  ...
  inquiryType: z.enum(['availability', 'pricing', 'technical', 'regulatory', 'other'], {
    message: 'Please select an inquiry type.',
  }),
  productId: z
    .string()
...
function getLocalizedMessage(locale: 'en' | 'ar', key: 'success' | 'error'): string {
  if (locale === 'ar') {
    return key === 'success'
...
function mapFormType(type: 'CONTACT' | 'PARTNERSHIP' | 'PRODUCT_INQUIRY'): FormType {
  if (type === 'PARTNERSHIP') {
...
function validationState(
  ...
  return {
    success: false,
    message: locale === 'ar' ? 'يرجى مراجعة الحقول المحددة.' : 'Please review the highlighted fields.',
    fieldErrors: Object.entries(fieldErrors).reduce<Record<string, string>>((acc, [key, value]) => {
      if (value && value[0]) {
        acc[key] = value[0];
      }
