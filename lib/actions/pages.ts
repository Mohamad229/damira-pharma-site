 ...
 */
export async function getPages(page: number = 1, pageSize: number = 20): Promise<ActionState<{
  pages: PageListItem[];
  total: number;
  pageCount: number;
}>> {
 ...
 */
export async function getPageById(id: string): Promise<ActionState<PageDetail>> {
  try {
    await requireAuth();
 ...
 */
export async function createPage(input: PageCreateInput): Promise<ActionState<PageDetail>> {
  try {
    await requireAuth();
 ...
 */
export async function updatePage(input: PageUpdateInput): Promise<ActionState<PageDetail>> {
  try {
    await requireAuth();
 ...
 */
export async function reorderSections(input: SectionReorderInput): Promise<ActionState> {
  try {
    await requireAuth();
 ...
 */
export async function getSectionById(id: string): Promise<ActionState<SectionDetail>> {
  try {
    await requireAuth();
    ...
    const section = await db.$transaction(async (tx) => {
      ...
      await tx.pageSectionTranslation.create({
        data: {
          ...
          locale: 'en',
          content: (validatedInput.content ?? { title: validatedInput.title ?? '' }) as Prisma.InputJsonValue,
        },
      });
    ...
    const updatedSection = await db.$transaction(async (tx) => {
      ...
      await tx.pageSection.update({
        ...
        data: {
          ...
          data: (validatedInput.data ?? {}) as Prisma.InputJsonValue | undefined,
        },
      });
      ...
      if (validatedInput.title || validatedInput.data) {
        ...
        if (translation) {
          const currentContent = (translation.content as Record<string, unknown> | null) ?? {};
          const newContent: Record<string, unknown> = {
            ...currentContent,
            title: validatedInput.title ?? (currentContent as Record<string, unknown>)?.title,
          };
 ...
 */
export async function duplicateSection(id: string): Promise<ActionState<SectionDetail>> {
  try {
    await requireAuth();
 ...
 */
export async function getSectionTypes(): Promise<ActionState<typeof SECTION_TYPE_OPTIONS>> {
  try {
