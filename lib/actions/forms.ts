...
export type FormSubmissionFilterOptions = z.infer<typeof FormSubmissionFilterSchema>;
...
export async function markFormSubmissionReviewed(id: string): Promise<ActionState> {
...
export async function getFormSubmissionStats(): Promise<ActionState<FormSubmissionStats>> {
  try {
    await requireAuth();
...
export async function getNewSubmissionsCount(): Promise<ActionState<{ count: number }>> {
  try {
    await requireAuth();
