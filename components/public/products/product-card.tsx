...
export function ProductCard({ product, locale, view, labels }: ProductCardProps) {
  const isRtl = locale === 'ar';
  const statusLabel = product.status === 'AVAILABLE' ? labels.available : labels.pipeline;
  ...
  return (
    ...
    >
      ...
      >
        {product.coverImageUrl ? (
          <Image
            ...
            alt={product.name}
            fill
            sizes={view === 'grid' ? '(max-width: 1024px) 100vw, 33vw' : '(max-width: 768px) 100vw, 320px'}
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <h2 className="mt-3 text-xl font-semibold leading-tight text-foreground">{product.name}</h2>
        ...
        >
          <div>
            <dt className="font-semibold text-foreground/85">{labels.category}</dt>
            <dd>{product.category?.name || '-'}</dd>
          </div>
          <div>
            <dt className="font-semibold text-foreground/85">{labels.therapeuticArea}</dt>
