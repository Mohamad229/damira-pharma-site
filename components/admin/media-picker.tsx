...
export function MediaPicker({
  ...
  const [typeFilter, setTypeFilter] = useState<FilterType>(accept === 'all' ? 'all' : accept);
  const [currentPage, setCurrentPage] = useState(1);
  const [data, setData] = useState<ClientPaginatedMedia | null>(null);
  const [selectedItems, setSelectedItems] = useState<Map<string, MediaWithUser>>(new Map());
  ...
  const loadMedia = useCallback(
    async (params: GetMediaOptions = {}) => {
      startTransition(async () => {
        const effectiveType = accept !== 'all' 
          ? accept 
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col p-0">
        ...
        <div className="p-6 pb-4 border-b border-border">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            {/* Upload Button */}
            <Button variant="outline" size="sm" onClick={() => setShowUploadZone(!showUploadZone)}>
  ...
  return (
    ...
    >
      ...
      <div className="p-3 space-y-1">
        <h4 className="text-xs font-medium text-foreground truncate" title={media.name}>
          {media.name}
        </h4>
        <div className="flex items-center justify-between text-[10px] text-muted-foreground">
          <span className="uppercase tracking-wide font-medium">{media.type}</span>
...
function PickerEmptyState({ onUpload }: { onUpload: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <h4 className="text-base font-semibold text-foreground mb-1">No media found</h4>
