 ...
 */
export function TiptapImageModal({ open, onOpenChange, onInsert }: ImageModalProps) {
  const [showMediaPicker, setShowMediaPicker] = useState(true);
  const [imageWidth, setImageWidth] = useState<ImageWidth>('100');
  const [imageAlignment, setImageAlignment] = useState<ImageAlignment>('center-block');
  const [selectedImage, setSelectedImage] = useState<MediaWithUser | null>(null);
  ...
  const handleConfirm = () => {
    ...
    const imageTag = buildImageTag(selectedImage.url, selectedImage.name, widthPercent, imageAlignment);
      ...
      {!showMediaPicker && selectedImage && (
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Configure how the image appears in your content</DialogDescription>
          </DialogHeader>
