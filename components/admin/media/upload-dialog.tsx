...
export function UploadDialog({ open, onOpenChange, onUploadComplete }: UploadDialogProps) {
  ...
  const handleClose = () => {
    ...
    if (hasUploading) {
      const confirmed = window.confirm('Upload in progress. Are you sure you want to close?');
      if (!confirmed) return;
    }
    setFiles([]);
    onOpenChange(false);
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload images, documents, or videos. Maximum file size: {formatFileSize(MAX_FILE_SIZE)}
          </DialogDescription>
        </DialogHeader>
