...
import { Grid3x3, List, Search, Upload, Loader2, FileText, Image as ImageIcon, Video } from 'lucide-react';
...
export function MediaLibrary({ initialData }: MediaLibraryProps) {
  ...
  const [typeFilter, setTypeFilter] = useState<GetMediaListParams['type'] | 'all'>('all');
  ...
  const loadMedia = useCallback(async (params: GetMediaListParams = {}) => {
    setIsLoading(true);
    try {
  ...
  const handleDelete = async (media: MediaWithUser) => {
    ...
    if (result.success) {
      ...
      loadMedia({ search: searchQuery, type: typeFilter === 'all' ? undefined : typeFilter });
    } else {
      toast({
        title: 'Error',
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
      <div className="flex items-center justify-center size-20 rounded-full bg-muted mb-6">
        <ImageIcon className="size-10 text-muted-foreground" strokeWidth={1.5} />
      </div>
