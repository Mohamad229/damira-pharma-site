...
import { cn, formatFileSize, formatDate, formatDateTime, copyToClipboard } from '@/lib/utils';

import type { MediaWithUser, MediaType, GetMediaOptions } from '@/lib/actions/media';
...
export function MediaLibraryClient({ initialData }: MediaLibraryClientProps) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (searchQuery !== '' || typeFilter !== 'all') {
        loadMedia({ search: searchQuery, type: typeFilter === 'all' ? undefined : typeFilter, page: 1 });
        setCurrentPage(1);
      }
    }, 300);
  ...
  const handleDelete = async (media: MediaWithUser) => {
    const confirmed = window.confirm(`Are you sure you want to delete "${media.name}"?`);
    if (!confirmed) return;
      ...
      {files.length > 0 && (
        <div className="px-4 pb-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={uploadAllFiles} disabled={isUploading || files.every((f) => f.status !== 'pending')}>
              {isUploading ? (
                <>
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-gradient-to-br from-muted/50 to-muted">
        {isImage && !imageError ? (
          ...
          <div className="flex size-full items-center justify-center">
            {isVideo ? (
              <Video className="size-16 text-muted-foreground/40" strokeWidth={1.5} />
            ) : (
              <FileText className="size-16 text-muted-foreground/40" strokeWidth={1.5} />
            )}
          </div>
        )}
      ...
      <div className="p-4 space-y-2">
        <h3 className="font-medium text-sm text-foreground truncate" title={media.name}>
          {media.name}
        </h3>
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span className="uppercase tracking-wide font-medium text-primary/80">{media.type}</span>
          <span>{formatFileSize(media.size)}</span>
        </div>
        <div className="text-xs text-muted-foreground">{formatDate(media.createdAt)}</div>
      </div>
    </Card>
  );
}
          ...
          <tr>
            <th className="w-12 p-4">
              <label className="relative flex items-center justify-center cursor-pointer">
                ...
                />
                {allSelected && <Check className="absolute size-2.5 text-white" />}
              </label>
            </th>
...
function MediaListRow({
  ...
  return (
    ...
    >
      ...
      <td className="p-4">
        <span className="text-sm text-muted-foreground">{formatDate(media.createdAt)}</span>
      </td>
      <td className="p-4">
        <div className="flex items-center justify-center gap-1">
          <Button variant="ghost" size="icon-sm" onClick={() => onPreview(media)} aria-label="Preview">
            <Eye className="size-4" />
          </Button>
          <Button variant="ghost" size="icon-sm" onClick={() => onCopyUrl(media.url)} aria-label="Copy URL">
            <Copy className="size-4" />
          </Button>
          <Button
            variant="destructive"
    ...
    <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
      <div className="relative mb-8">
        ...
        <div className="relative flex items-center justify-center size-24 rounded-2xl bg-gradient-to-br from-muted to-muted/50 border border-border">
          <ImageIcon className="size-12 text-muted-foreground/40" strokeWidth={1.5} />
        </div>
      </div>
      ...
      {hasFilters ? (
        ...
        <>
          ...
          <p className="text-sm text-muted-foreground mb-6 max-w-md">
            Upload images, documents, or videos to get started with your media library
          </p>
        ...
        <div className="relative flex-1 min-h-[300px] bg-gradient-to-br from-muted/50 via-muted to-muted/50 flex items-center justify-center overflow-hidden">
          {isImage ? (
            ...
            <div className="flex flex-col items-center gap-4">
              <FileText className="size-24 text-muted-foreground/40" strokeWidth={1} />
        ...
        <div className="p-6 border-t border-border bg-card">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
            ...
            <div className="space-y-4 flex-1">
              <div>
                <h2 className="text-lg font-semibold text-foreground">{media.name}</h2>
            ...
            <div className="flex sm:flex-col gap-2">
              <Button variant="outline" onClick={() => onDownload(media)} className="flex-1">
