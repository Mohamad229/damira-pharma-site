...
import { ChevronDown, ChevronUp, AlertCircle, Loader2, Trash2, ArrowLeft } from 'lucide-react';
...
import { generateSlug, PageCreateSchema, PageUpdateSchema } from '@/lib/schemas/page-form';
...
export function PageFormClient({ mode, pageId, initialData }: PageFormClientProps) {
  ...
  const [isLoadingData, setIsLoadingData] = useState(mode === 'edit' && !initialData);
  ...
  const [unsavedByLanguage, setUnsavedByLanguage] = useState<{ en: boolean; ar: boolean }>({
    en: false,
    ar: false,
  });
  ...
  const handleInputChange = useCallback(
    (field: string, value: string | boolean) => {
      if (field === 'slug') {
        ...
        setFormData((prev) => {
          const updated = { ...prev };
          updated[currentLanguage] = { ...updated[currentLanguage], title: value as string };
  ...
  const validateForm = (): boolean => {
    ...
    if (currentLangData.metaDescription && currentLangData.metaDescription.length > 160) {
          ...
          <div className="bg-muted/50 rounded-lg p-4 text-sm text-muted-foreground">
            ...
            <div className="space-y-1">
              <p className="text-primary font-medium truncate">
                {formData[currentLanguage].metaTitle || formData[currentLanguage].title}
              </p>
