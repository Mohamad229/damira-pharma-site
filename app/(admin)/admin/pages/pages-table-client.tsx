...
export function PagesTableClient({ initialData }: PagesTableClientProps) {
    ...
    startTransition(async () => {
      ...
      if (result.data) {
        ...
        const filtered = result.data.pages.filter((page) => {
          ...
          const title = page.translations.find((t) => t.locale === 'en')?.title || '';
          return title.toLowerCase().includes(searchLower) || page.slug.toLowerCase().includes(searchLower);
        });
  ...
  const handleSortingChange = useCallback((sortingUpdater: SortingState | ((old: SortingState) => SortingState)) => {
    ...
    startTransition(async () => {
      ...
      if (result.data) {
        ...
        if (newSorting[0]?.id === 'title') {
          sorted.sort((a, b) => {
            const aTitle = a.translations.find((t) => t.locale === 'en')?.title || '';
            const bTitle = b.translations.find((t) => t.locale === 'en')?.title || '';
            return newSorting[0]?.desc
              ? bTitle.localeCompare(aTitle)
              : aTitle.localeCompare(bTitle);
          });
        } else if (newSorting[0]?.id === 'slug') {
          sorted.sort((a, b) => (newSorting[0]?.desc ? b.slug.localeCompare(a.slug) : a.slug.localeCompare(b.slug)));
        } else if (newSorting[0]?.id === 'createdAt') {
          sorted.sort((a, b) =>
            newSorting[0]?.desc ? b.createdAt.getTime() - a.createdAt.getTime() : a.createdAt.getTime() - b.createdAt.getTime()
          );
        }
  ...
  const SimpleCheckbox = ({ checked, onChange, disabled }: { checked: boolean; onChange: (value: boolean) => void; disabled?: boolean }) => (
    <button
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const title = page.translations.find((t) => t.locale === 'en')?.title || 'Untitled';
          return (
      ...
      },
      {
        ...
        header: 'Translations',
        cell: ({ row }) => {
          const locales = row.original.translations.map((t) => t.locale.toUpperCase()).join(', ');
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex items-center gap-2">
              ...
              <Link href={`/admin/pages/new?duplicate=${page.id}`}>
                <Button variant="ghost" size="sm" className="gap-2">
      ...
      <Dialog open={deleteConfirmId !== null} onOpenChange={(open) => !open && setDeleteConfirmId(null)}>
        <DialogContent showCloseButton>
          ...
          <DialogFooter>
            ...
            <Button
              variant="default"
              onClick={() => deleteConfirmId && handleDeletePage(deleteConfirmId)}
              disabled={isDeleting}
