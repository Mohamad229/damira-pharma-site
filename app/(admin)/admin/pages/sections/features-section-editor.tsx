...
export function FeaturesSectionEditor({
  ...
  const [layout, setLayout] = useState<'list' | 'grid'>(initialData?.layout || 'list');
  ...
  const errors = useMemo(() => {
    ...
    if (features.length === 0) errs.features = 'At least one feature is required';
    return errs;
  }, [title, features]);
  ...
  const updateFeature = (index: number, updates: Partial<Feature>) => {
    setFeatures(
      features.map((feature, i) => (i === index ? { ...feature, ...updates } : feature))
    );
  };
              ...
              <div className="flex-1">
                ...
                <div className="mt-1">
                  <TiptapEditor
                    ...
                    onChange={(html) => updateFeature(index, { description: html })}
            ...
            {layout === 'list' ? (
              <div className="space-y-2 mt-4">
                {features.slice(0, 3).map((feature) => (
                  <div key={feature.id} className="flex gap-3 p-2 rounded border border-border">
