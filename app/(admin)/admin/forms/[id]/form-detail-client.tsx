...
export function FormDetailClient({ submission }: FormDetailClientProps) {
  const [status, setStatus] = useState<FormSubmissionDetail['status']>(submission.status);
  ...
  const canSaveStatus = useMemo(() => status !== submission.status, [status, submission.status]);
        ...
        <CardContent className="space-y-5">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-foreground">{submission.name}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Email</p>
              <p className="mt-1 text-sm font-medium text-foreground">{submission.email}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Phone</p>
              <p className="mt-1 text-sm text-foreground">{submission.phone || '-'}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Company</p>
              <p className="mt-1 text-sm text-foreground">{submission.company || '-'}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Type</p>
              <p className="mt-1 text-sm text-foreground">{formatType(submission.type)}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Inquiry Type</p>
              <p className="mt-1 text-sm text-foreground">{submission.inquiryType || '-'}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Product ID</p>
              <p className="mt-1 break-all text-sm text-foreground">{submission.productId || '-'}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Submitted</p>
              <p className="mt-1 text-sm text-foreground">{formatDateTime(submission.createdAt)}</p>
            </div>
          </div>
      ...
      <div className="space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Change Status</p>
              <Select
                value={status}
                onChange={(event) => setStatus(event.target.value as FormSubmissionDetail['status'])}
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-muted-foreground">
              ...
              <p>Deleting removes this record permanently from admin history.</p>
            </div>
