...
type LocalizedNotFoundPageProps = {
  params: Promise<{ locale: string }>;
};

export default async function LocalizedNotFoundPage({ params }: LocalizedNotFoundPageProps) {
  const { locale } = await params;
  const tCommon = await getTranslations({ locale, namespace: 'common' });
  const tError = await getTranslations({ locale, namespace: 'errors' });

  return (
    <div className="relative mx-auto flex min-h-[70vh] w-full max-w-4xl flex-col items-center justify-center overflow-hidden px-4 text-center">
      ...
      <div className="rounded-[2rem] border border-border/80 bg-card/95 px-8 py-10 shadow-[var(--shadow-card)]">
        ...
        <h1 className="mt-4 text-2xl font-semibold text-foreground">{tError('notFoundTitle')}</h1>
        <p className="mt-2 max-w-md text-sm text-muted-foreground">{tError('notFoundDescription')}</p>
      </div>
